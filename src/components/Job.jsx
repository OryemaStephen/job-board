...
const Job = ({ id, company, title, logo, salary, description, type, location }) => {
  return (
    <Link to={`/searchjobs/${id}`} className='w-full p-4 bg-white rounded-lg shadow-lg'>
      ...
      <div className='flex justify-between gap-4'>
        ...
        <span className='px-2 py-1 text-sm font-semibold rounded'>{location}</span>
      </div>
    </Link>
  );
};
